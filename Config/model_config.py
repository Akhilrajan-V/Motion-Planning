...
class RasterizedPlanningModel(nn.Module):
    ...
    def __init__(
        ...
        if model_arch == "resnet18":
            ...
            self.model = googlenet(pretrained=pretrained)
            self.model.fc = nn.Linear(in_features=2048, out_features=num_targets)
        else:
            raise NotImplementedError(f"Model arch {model_arch} unknown")

        if self.num_input_channels != 3:
            if self.model_arch == "vit16":
                self.model.conv_proj = torch.nn.Conv2d(self.num_input_channels, 768, kernel_size=(16, 16),
                                                       stride=(16, 16))
            elif self.model_arch == "googlenet":
                self.model.conv1 = nn.Conv2d(self.num_input_channels, self.model.conv1.conv.out_channels,
                                             kernel_size=self.model.conv1.conv.kernel_size,
                                             stride=self.model.conv1.conv.stride, padding=self.model.conv1.conv.padding,
                                             bias=False, )
            else:
